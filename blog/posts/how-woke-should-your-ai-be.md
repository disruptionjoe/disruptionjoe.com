...
Imagine an AI model operating with zero constraints. It would function like an idealized philosopher or scientist: processing data, analyzing evidence, and presenting conclusions based on the best available information. In this form, AI would be a pure intellectual tool, free from societal or moral filters.
...
1. Model-Level Constraints (Before Deployment)

Some constraints are built into the model itself, often by its creators. For instance, an AI might be programmed never to discuss harmful topics, such as instructions for creating weapons or engaging in illegal activities. These constraints are often informed by universal ethical principles—like the need to prevent harm.

When a base model is open-source, as with Meta's Llama, these constraints become less enforceable. Developers can freely modify the model, raising concerns about misuse. Transparency in model training, such as the emerging practice of "proof of inference," could help monitor and verify how these models are deployed.

2. Community-Level Constraints (After Deployment)

Once a model is released into the wild, its behavior can be further shaped by the communities that use it. Communities can define their own ethical boundaries, tailored to their values, cultural norms, or practical needs. For example:
...
A nation-state might implement constraints aligned with its laws, such as banning discussions about hate speech or promoting political propaganda.
...
1. Single Administrator (EOA Model)

In this model, a single person or entity holds decision-making power. This is akin to having a single "keyholder" for an Ethereum wallet. While efficient, it risks bias or misuse of power—like an authoritarian figure deciding how "woke" the AI should be.

2. Multi-Signature Approval (Multi-Sig Model)

Here, decisions require approval from multiple stakeholders. For example, a council of representatives from different groups might vote on proposed constraints. This model adds a layer of accountability and reduces the risk of unilateral decisions.

3. Community Voting (DAO Model)
...
In the end, the ideal AI governance system will likely combine multiple approaches: transparency at the model level, community-driven customization, and mechanisms for participatory decision-making. Whether it's a single administrator, a multi-sig council, or a decentralized community, the goal is the same: to ensure that AI serves humanity responsibly while respecting the diverse needs and values of its users.

So, how woke should your AI be? The answer lies in your hands—and the collective hands of the communities shaping its future.