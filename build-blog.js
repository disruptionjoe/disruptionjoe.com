#!/usr/bin/env node
/**
 * build-blog.js — Markdown-to-HTML blog builder for disruptionjoe.com
 *
 * Reads .md files from blog/posts/, renders them to static HTML in blog/,
 * and generates a blog index page. Matches the existing site style.
 *
 * Usage: node build-blog.js
 *
 * Dependencies: npm install marked gray-matter
 */

const fs = require('fs');
const path = require('path');
const matter = require('gray-matter');
const { marked } = require('marked');

const POSTS_DIR = path.join(__dirname, 'blog', 'posts');
const OUTPUT_DIR = path.join(__dirname, 'blog');

// --- Shared site styles (extracted from the live site) ---

const SITE_STYLES = `
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }

    :root {
      --black: #0a0a0a;
      --white: #f5f5f5;
      --cream: #f0ece4;
      --gray: #888;
      --gray-dim: #555;
      --gray-dark: #1a1a1a;
      --gray-border: #222;
      --tan: #d4bc94;
    }

    html { scroll-behavior: smooth; }

    body {
      font-family: 'Space Grotesk', -apple-system, sans-serif;
      background: var(--black);
      color: var(--white);
      line-height: 1.6;
      -webkit-font-smoothing: antialiased;
    }

    /* NAV */
    nav {
      position: fixed;
      top: 0;
      width: 100%;
      padding: 1.5rem 3rem;
      display: flex;
      justify-content: space-between;
      align-items: center;
      z-index: 100;
      background: rgba(10,10,10,0.95);
      backdrop-filter: blur(20px);
      border-bottom: 1px solid var(--gray-border);
    }
    .nav-logo {
      font-family: 'Space Grotesk', sans-serif;
      font-weight: 700;
      font-size: 1.15rem;
      letter-spacing: -0.03em;
      color: var(--white);
      text-decoration: none;
    }
    .nav-links { display: flex; gap: 2rem; align-items: center; }
    .nav-links a {
      color: var(--gray);
      text-decoration: none;
      font-size: 0.85rem;
      font-weight: 500;
      transition: color 0.3s;
    }
    .nav-links a:hover { color: var(--white); }
    .nav-links a.active { color: var(--tan); }
    .nav-cta {
      color: var(--white) !important;
      border: 1.5px solid #444 !important;
      padding: 0.5rem 1.25rem;
      font-size: 0.8rem !important;
      font-weight: 600 !important;
      transition: all 0.3s !important;
    }
    .nav-cta:hover {
      border-color: #888 !important;
    }

    /* CONTENT */
    .page-container {
      max-width: 720px;
      margin: 0 auto;
      padding: 8rem 2rem 4rem;
    }

    /* BLOG INDEX */
    .blog-header h1 {
      font-family: 'Space Grotesk', sans-serif;
      font-size: clamp(2rem, 4vw, 3rem);
      font-weight: 700;
      letter-spacing: -0.04em;
      margin-bottom: 0.5rem;
    }
    .blog-header p {
      color: var(--gray);
      font-size: 1.05rem;
      margin-bottom: 3rem;
    }
    .post-list { list-style: none; }
    .post-list li {
      border-bottom: 1px solid var(--gray-border);
      padding: 1.5rem 0;
    }
    .post-list li:first-child { border-top: 1px solid var(--gray-border); }
    .post-list a { text-decoration: none; display: block; }
    .post-list .post-title {
      font-size: 1.2rem;
      font-weight: 600;
      color: var(--white);
      margin-bottom: 0.3rem;
      transition: color 0.3s;
    }
    .post-list a:hover .post-title { color: var(--tan); }
    .post-list .post-meta {
      font-size: 0.8rem;
      color: var(--gray-dim);
      font-family: 'Space Mono', monospace;
    }
    .post-list .post-desc {
      color: var(--gray);
      font-size: 0.9rem;
      margin-top: 0.4rem;
      line-height: 1.5;
    }

    /* ARTICLE */
    .article-header {
      margin-bottom: 3rem;
      padding-bottom: 2rem;
      border-bottom: 1px solid var(--gray-border);
    }
    .article-header h1 {
      font-family: 'Space Grotesk', sans-serif;
      font-size: clamp(1.8rem, 4vw, 2.8rem);
      font-weight: 700;
      letter-spacing: -0.03em;
      line-height: 1.15;
      margin-bottom: 1rem;
    }
    .article-meta {
      font-size: 0.8rem;
      color: var(--gray-dim);
      font-family: 'Space Mono', monospace;
    }
    .article-body {
      font-size: 1.05rem;
      line-height: 1.8;
      color: var(--cream);
    }
    .article-body h2 {
      font-size: 1.4rem;
      font-weight: 700;
      margin-top: 2.5rem;
      margin-bottom: 1rem;
      color: var(--white);
    }
    .article-body h3 {
      font-size: 1.15rem;
      font-weight: 600;
      margin-top: 2rem;
      margin-bottom: 0.8rem;
      color: var(--white);
    }
    .article-body p {
      margin-bottom: 1.2rem;
    }
    .article-body ul, .article-body ol {
      margin-bottom: 1.2rem;
      padding-left: 1.5rem;
    }
    .article-body li {
      margin-bottom: 0.4rem;
    }
    .article-body blockquote {
      border-left: 3px solid var(--tan);
      padding-left: 1.2rem;
      margin: 1.5rem 0;
      color: var(--gray);
      font-style: italic;
    }
    .article-body a {
      color: var(--tan);
      text-decoration: none;
      border-bottom: 1px solid transparent;
      transition: border-color 0.3s;
    }
    .article-body a:hover {
      border-bottom-color: var(--tan);
    }
    .article-body em {
      color: var(--white);
    }
    .article-body strong {
      color: var(--white);
      font-weight: 600;
    }
    .article-body hr {
      border: none;
      border-top: 1px solid var(--gray-border);
      margin: 2rem 0;
    }
    .back-link {
      display: inline-block;
      margin-top: 3rem;
      color: var(--gray);
      text-decoration: none;
      font-size: 0.85rem;
      transition: color 0.3s;
    }
    .back-link:hover { color: var(--tan); }

    /* FOOTER */
    footer {
      text-align: center;
      padding: 3rem 2rem;
      color: var(--gray-dim);
      font-size: 0.8rem;
      border-top: 1px solid var(--gray-border);
      margin-top: 4rem;
    }

    /* MOBILE */
    @media (max-width: 768px) {
      nav { padding: 1rem 1.5rem; }
      .nav-links { display: none; }
      .page-container { padding: 6rem 1.5rem 3rem; }
    }
`;

const NAV_HTML = `
  <nav>
    <a href="/" class="nav-logo">Disruption Joe</a>
    <div class="nav-links">
      <a href="/#how-it-works">How it works</a>
      <a href="/about.html">About me</a>
      <a href="/blog/" class="active">Blog</a>
      <a href="/discovery/">For Teams</a>
      <a href="/pricing.html">Pricing</a>
      <a href="/start.html" class="nav-cta">Book a session</a>
    </div>
  </nav>
`;

const FOOTER_HTML = `
  <footer>
    &copy; ${new Date().getFullYear()} Disruption Joe. All rights reserved.
  </footer>
`;

// --- Helpers ---

function slugFromFilename(filename) {
  return filename.replace(/\.md$/, '');
}

function formatDate(dateStr) {
  const d = new Date(dateStr);
  return d.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
}

function postTemplate(post) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${post.title} | Disruption Joe</title>
  <meta name="description" content="${post.description || ''}">
  <meta name="author" content="Disruption Joe">
  <meta name="robots" content="index, follow">
  <meta property="og:type" content="article">
  <meta property="og:url" content="https://disruptionjoe.com/blog/${post.slug}.html">
  <meta property="og:title" content="${post.title}">
  <meta property="og:description" content="${post.description || ''}">
  <meta property="og:site_name" content="Disruption Joe">
  <meta property="og:image" content="https://disruptionjoe.com/og-image.jpg">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="${post.title}">
  <meta name="twitter:description" content="${post.description || ''}">
  <link rel="canonical" href="https://disruptionjoe.com/blog/${post.slug}.html">
  <meta name="theme-color" content="#0a0a0a">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&family=Space+Mono:wght@400;700&display=swap" rel="stylesheet">
  <style>${SITE_STYLES}</style>
</head>
<body>
${NAV_HTML}
  <div class="page-container">
    <div class="article-header">
      <h1>${post.title}</h1>
      <div class="article-meta">${post.dateFormatted}${post.author ? ' · ' + post.author : ''}</div>
    </div>
    <div class="article-body">
      ${post.html}
    </div>
    <a href="/blog/" class="back-link">&larr; Back to all posts</a>
  </div>
${FOOTER_HTML}
</body>
</html>`;
}

function indexTemplate(posts) {
  const listItems = posts.map(p => `
      <li>
        <a href="/blog/${p.slug}.html">
          <div class="post-title">${p.title}</div>
          <div class="post-meta">${p.dateFormatted}</div>
          ${p.description ? `<div class="post-desc">${p.description}</div>` : ''}
        </a>
      </li>`).join('\n');

  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Blog | Disruption Joe</title>
  <meta name="description" content="Thinking about AI, coordination, and where technology actually fits your work.">
  <meta name="author" content="Disruption Joe">
  <meta name="robots" content="index, follow">
  <meta property="og:type" content="website">
  <meta property="og:url" content="https://disruptionjoe.com/blog/">
  <meta property="og:title" content="Blog | Disruption Joe">
  <meta property="og:description" content="Thinking about AI, coordination, and where technology actually fits your work.">
  <meta property="og:site_name" content="Disruption Joe">
  <meta property="og:image" content="https://disruptionjoe.com/og-image.jpg">
  <link rel="canonical" href="https://disruptionjoe.com/blog/">
  <meta name="theme-color" content="#0a0a0a">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@400;500;600;700&family=Space+Mono:wght@400;700&display=swap" rel="stylesheet">
  <style>${SITE_STYLES}</style>
</head>
<body>
${NAV_HTML}
  <div class="page-container">
    <div class="blog-header">
      <h1>Blog</h1>
      <p>Thinking about AI, coordination, and where technology actually fits your work.</p>
    </div>
    <ul class="post-list">
${listItems}
    </ul>
  </div>
${FOOTER_HTML}
</body>
</html>`;
}

// --- Main ---

function main() {
  // Collect all .md files from posts dir
  const files = fs.readdirSync(POSTS_DIR).filter(f => f.endsWith('.md') && f !== 'INDEX.md');

  const posts = files.map(file => {
    const raw = fs.readFileSync(path.join(POSTS_DIR, file), 'utf-8');
    const { data, content } = matter(raw);

    // For posts without frontmatter, try to extract title from first # heading
    let title = data.title;
    let description = data.description || '';
    let date = data.date;
    let author = data.author || 'Disruption Joe';

    let bodyContent = content;

    if (!title) {
      const titleMatch = content.match(/^#\s+(.+)$/m);
      title = titleMatch ? titleMatch[1] : slugFromFilename(file).replace(/-/g, ' ');
      // Remove the title line from body
      bodyContent = content.replace(/^#\s+.+\n/, '');
    }

    if (!date) {
      // Try to extract date from byline like "*By Joe ... · January 2025*"
      const dateMatch = content.match(/\*.*?·\s*(.+?)\*/);
      if (dateMatch) {
        date = dateMatch[1].trim();
        // Remove the byline from body
        bodyContent = bodyContent.replace(/\*By.*?\*\n*/, '');
      } else {
        date = '2025-01-01';
      }
    }

    // Remove leading horizontal rules
    bodyContent = bodyContent.replace(/^\s*---\s*\n/, '');

    const slug = slugFromFilename(file);
    const html = marked(bodyContent);

    let dateObj;
    if (typeof date === 'string' && date.match(/^\d{4}-\d{2}/)) {
      dateObj = new Date(date);
    } else if (typeof date === 'object' && date instanceof Date) {
      dateObj = date;
    } else {
      // Parse natural dates like "January 2025"
      dateObj = new Date(date);
    }

    const dateFormatted = formatDate(dateObj);

    return { title, description, date: dateObj, dateFormatted, author, slug, html };
  });

  // Sort newest first
  posts.sort((a, b) => b.date - a.date);

  // Write individual post pages
  let written = 0;
  for (const post of posts) {
    const outPath = path.join(OUTPUT_DIR, `${post.slug}.html`);
    fs.writeFileSync(outPath, postTemplate(post));
    written++;
    console.log(`  ✓ ${post.slug}.html`);
  }

  // Write index page
  const indexPath = path.join(OUTPUT_DIR, 'index.html');
  fs.writeFileSync(indexPath, indexTemplate(posts));
  console.log(`  ✓ index.html (${posts.length} posts)`);

  console.log(`\nBlog built: ${written} posts + index page.`);
}

main();
